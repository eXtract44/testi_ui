...
void moving_median_create(movingMedian_t *context, uint16_t filter_size,
...
void moving_median_filter(movingMedian_t *context, int16_t filter_input) {
	if ((TICK_TIMER - context->last_time) > context->sample_time) {
		...
		int16_t *sort_buffer = (int16_t*) malloc(
				context->fill * sizeof(uint16_t));
