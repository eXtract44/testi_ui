...
//-------------------------------------------------------------------
__STATIC_INLINE void delaymic(uint32_t __IO micros) {
	micros *= (SystemCoreClock / 1720000) / 5;
	while (micros--)
		;
}
void draw_button(uint16_t x, uint16_t y, uint16_t color, uint16_t bg, char c[]) {
	...
	//ILI9341_string(x + 22, y + 5, c, color, bg);
}
//-------------------------------------------------------------------
...
//-------------------------------------------------------------------
void draw_menu_antrieb_s() { //Antriebtester S
	HAL_GPIO_WritePin(GPIOB, V24ON1_Pin, GPIO_PIN_SET); //24VINTERN
	HAL_GPIO_WritePin(GPIOB, V24ON2_Pin, GPIO_PIN_SET); //24VOUT
	ILI9341_SetBrightness(1);
	ILI9341_fill(COLOR_MENU_BG);
...
//}
//-------------------------------------------------------------------
void draw_menu_raumregler_aktiv() { //sensor menu
	//HAL_GPIO_WritePin(GPIOB, V24ON1_Pin, GPIO_PIN_SET); //24VINTERN
	HAL_GPIO_WritePin(GPIOB, V24ON2_Pin, GPIO_PIN_SET); //24VOUT
	ILI9341_SetBrightness(1);
	ILI9341_fill(COLOR_MENU_BG);
...
//-------------------------------------------------------------------
void draw_menu_raumregler_passiv() { //sensor menu
	HAL_GPIO_WritePin(GPIOB, V24ON1_Pin, GPIO_PIN_SET); //24VINTERN
	HAL_GPIO_WritePin(GPIOB, V24ON2_Pin, GPIO_PIN_SET); //24VOUT
	ILI9341_SetBrightness(1);
	ILI9341_fill(COLOR_MENU_BG);
 ...
 */
//-------------------------------------------------------------------
void draw_menu_kanaltemp() { //sensor menu1
	//HAL_GPIO_WritePin(GPIOB, V24ON1_Pin, GPIO_PIN_SET); //24VINTERN
	HAL_GPIO_WritePin(GPIOB, V24ON2_Pin, GPIO_PIN_SET); //24VOUT
	ILI9341_SetBrightness(1);
	ILI9341_fill(COLOR_MENU_BG);
...
//-------------------------------------------------------------------
void draw_menu_druckluft() { //sensor menu1
	//HAL_GPIO_WritePin(GPIOB, V24ON1_Pin, GPIO_PIN_SET); //24VINTERN
	HAL_GPIO_WritePin(GPIOB, V24ON2_Pin, GPIO_PIN_SET); //24VOUT
	ILI9341_SetBrightness(1);
	ILI9341_fill(COLOR_MENU_BG);
...
//-------------------------------------------------------------------
void draw_menu_druckwasser() { //sensor menu1
	//HAL_GPIO_WritePin(GPIOB, V24ON1_Pin, GPIO_PIN_SET); //24VINTERN
	HAL_GPIO_WritePin(GPIOB, V24ON2_Pin, GPIO_PIN_SET); //24VOUT
	ILI9341_SetBrightness(1);
	ILI9341_fill(COLOR_MENU_BG);
...
//-------------------------------------------------------------------
void draw_menu_luftquali() { //sensor menu1
	//HAL_GPIO_WritePin(GPIOB, V24ON1_Pin, GPIO_PIN_SET); //24VINTERN
	HAL_GPIO_WritePin(GPIOB, V24ON2_Pin, GPIO_PIN_SET); //24VOUT
	ILI9341_SetBrightness(1);
	ILI9341_fill(COLOR_MENU_BG);
...
//-------------------------------------------------------------------
void draw_menu_helligkeit() { //sensor menu1
	//HAL_GPIO_WritePin(GPIOB, V24ON1_Pin, GPIO_PIN_SET); //24VINTERN
	HAL_GPIO_WritePin(GPIOB, V24ON2_Pin, GPIO_PIN_SET); //24VOUT
	ILI9341_SetBrightness(1);
	ILI9341_fill(COLOR_MENU_BG);
...
//-------------------------------------------------------------------
void draw_menu_ysensor() { //sensor menu1
	//HAL_GPIO_WritePin(GPIOB, V24ON1_Pin, GPIO_PIN_SET); //24VINTERN
	HAL_GPIO_WritePin(GPIOB, V24ON2_Pin, GPIO_PIN_SET); //24VOUT
	ILI9341_SetBrightness(1);
	ILI9341_fill(COLOR_MENU_BG);
...
//-------------------------------------------------------------------
void draw_premenu_ysensor() { //sensor menu1
	ILI9341_SetBrightness(1);
	ILI9341_fill(COLOR_MENU_BG);
...
//-------------------------------------------------------------------
void draw_pt100menu() { //sensor menu
	HAL_GPIO_WritePin(GPIOB, V24ON1_Pin, GPIO_PIN_SET); //24VINTERN
	//HAL_GPIO_WritePin(GPIOB, V24ON2_Pin, GPIO_PIN_SET); //24VOUT
	ILI9341_SetBrightness(1);
	ILI9341_fill(COLOR_MENU_BG);
...
//-------------------------------------------------------------------
void select_main_menu(uint8_t menu_page) { //draw_select_menu
...
//-------------------------------------------------------------------
void select_sensor_submenu(uint8_t menu_page) { //draw_select_menu
	uint16_t color_block = COLOR_MENU_BLOCK;
	uint16_t color_curr_block = COLOR_WHITE;
	uint16_t y_block_left_right = 100;
	uint16_t y_block_mid = y_block_left_right - 10;
...
//-------------------------------------------------------------------
void select_antrieb_submenu(uint8_t menu_page) { //draw_select_menu
	uint16_t color_block = COLOR_MENU_BLOCK;
	uint16_t color_curr_block = COLOR_WHITE;
	uint16_t y_block_left_right = 100;
	uint16_t y_block_mid = y_block_left_right - 10;
...
//-------------------------------------------------------------------
void draw_antriebAnim(uint16_t x, uint16_t y, int16_t val) {  //x=170,y=80
