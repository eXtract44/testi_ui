...
void read_adc_antrieb_r() {
	...
	//-----------------------------IN1
	u[adc_channel_in1] = ((float) adc_filter[ADC_CHANNEL_INPUT1_0_10V].filtered) * Vref / adcRes;
...
///**test Resistor ADC
//	u[adc_channel_in1] = (float) adc_filter[ADC_CHANNEL_INPUT1_0_10V].filtered; //test ADC
//	sprintf(str, "%3d%% ", (uint16_t) u[adc_channel_in1]);
//	ILI9341_printText(str, adc_val_xstart, adc_val_ystart, COLOR_WHITE,
//	COLOR_MENU_BG, 2);
///**test Resistor ADC
//-----------------------------in1 poti
	u[adc_channel_in1] = (float) adc_filter[ADC_CHANNEL_INPUT1_0_10V].filtered;
	...
	///**test PT100 ADC
//	u[adc_channel_pt100] = (float) adc_filter[ADC_CHANNEL_INPUT_PT100].filtered;
...
void read_adc_feuchte() {
	...
	if (measuring_range_feuchte == 1) {  //
		u[adc_channel_in2] = ((float) adc_filter[ADC_CHANNEL_INPUT2_0_10V].filtered) * Vref / adcRes;
...
void read_adc_channels_v(uint8_t channel, uint16_t x, uint16_t y) {
	...
	case 0: //24v
		...
		//u[adc_channel_24v] = ((float) adc_filter[ADC_CHANNEL_OUTPUT_24V].filtered) * Vref / adcRes;
...
//||
	...
	case 1:
		//y
		u[adc_channel_y] = ((float) adc_filter[ADC_CHANNEL_OUTPUT_0_10V].filtered) * Vref / adcRes;
...
void adc_filter_loop() {
	...
	moving_median_filter(&adc_filter[ADC_CHANNEL_INPUT_PT100], adc_data[adc_channel_pt100]);
////moving_median_filter(&adc_filter[ADC_CHANNEL_OUTPUT_24V], adc_data[adc_channel_24v]);
	moving_median_filter(&adc_filter[ADC_CHANNEL_OUTPUT_0_10V], adc_data[adc_channel_y]);
	moving_median_filter(&adc_filter[ADC_CHANNEL_INPUT1_0_10V], adc_data[adc_channel_in1]);
	moving_median_filter(&adc_filter[ADC_CHANNEL_INPUT2_0_10V], adc_data[adc_channel_in2]);
	moving_median_filter(&adc_filter[ADC_CHANNEL_MCU_TEMPERATURE], adc_data[adc_channel_temp]);
	moving_median_filter(&adc_filter[ADC_CHANNEL_BATTERY], adc_data[adc_channel_bat]);
